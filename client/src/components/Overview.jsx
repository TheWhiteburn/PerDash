...
function ladder(v, l=40, h=90) {
  ...
  return { cls: 'text-gray-700', fill: '#555', bg: 'bg-gray-600' };
}
...
export default function Overview() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      <div className="flex items-center gap-5 bg-[#f5f5f5] border border-black rounded-lg p-5 shadow-card-white">
        <svg viewBox="0 0 120 120" className="w-28 h-28 shrink-0">
          <circle cx="60" cy="60" r="54" fill="none" stroke="#d4d4d4" strokeWidth="8" />
          <circle
        ...
        <div>
          <div className="text-xs text-gray-500 tracking-wider uppercase mb-1">Overall Progress</div>
          {overall < 10 && <p className="text-gray-500 text-sm mt-1">Just started. Every day counts. Lock in.</p>}
          {overall >= 10 && overall < 30 && <p className="text-gray-500 text-sm mt-1">Building momentum. Stay consistent.</p>}
          {overall >= 30 && overall < 60 && <p className="text-gray-500 text-sm mt-1">Solid progress. Keep pushing.</p>}
          {overall >= 60 && <p className="text-gray-500 text-sm mt-1">Beast mode. Don't stop now.</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {metrics.map(m => {
          ...
          return (
            ...
            >
              <div className="text-[10px] text-gray-500 tracking-wider uppercase mb-2">{m.label}</div>
              <div className={`text-lg font-semibold ${mc.cls}`}>
                {m.value}
              </div>
            </div>
          );
      ...
      <div className="bg-[#f5f5f5] border border-black rounded-lg p-5 shadow-card-white">
        <div className="text-xs text-gray-500 tracking-wider uppercase mb-3">Current Focus</div>
        <p className="text-sm font-medium text-gray-900 mb-3">{topGoal?.text || 'No goals set'}</p>
        ...
        <div>
          ...
          <div className="space-y-2">
            {data.workouts.challenges.filter(c => c.active).map(c => {
              ...
              const cc = ladder(pct);
              return (
                <div key={c.id} className="flex items-center justify-between bg-[#f5f5f5] border border-black rounded-lg p-4 shadow-card-white">
