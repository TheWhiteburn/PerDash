          ...
          {items.map(item => (
            <label
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-md cursor-pointer transition-all duration-150 ${
                item.done ? 'bg-white/5' : 'bg-black border border-white/10 hover:border-gray-500'
              }`}
            >
              <input
      ...
      <div className="bg-[#0a0a0a] border border-white/25 rounded-lg p-5 shadow-card-white">
        ...
        <div className="space-y-1">
          {data.dailyChecklist.presets.map(p => (
            <label key={p.id} className="flex items-center gap-3 px-3 py-2 rounded-md cursor-pointer hover:bg-white/5 transition-colors">
              <input
                ...
                className="appearance-none w-4 h-4 border-2 rounded-sm cursor-pointer shrink-0 transition-all duration-150
                  ...
                  checked:after:content-['✓'] checked:after:text-[10px] checked:after:text-black checked:after:flex checked:after:items-center checked:after:justify-center"
              />
              <span className={`text-sm ${p.active ? 'text-gray-200' : 'text-gray-600 line-through'}`}>
                {p.text}
              </span>
            </label>
          ))}
