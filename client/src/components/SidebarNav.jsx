...
export default function SidebarNav({ active, onSelect }) {
  return (
    <nav className="w-14 md:w-44 bg-[#0a0a0a] border-r border-[#1e3a5f] flex flex-col items-center md:items-stretch gap-1.5 py-3 px-1 md:px-3 shrink-0 h-full overflow-y-auto">
        ...
        return (
          <button
            ...
            className={`flex items-center justify-center md:justify-start gap-3 px-2 md:px-3 py-2.5 rounded-md text-sm transition-all duration-150 ${
              isActive
                ? 'bg-[#1e3a5f]/40 text-blue-300 border border-[#1e3a5f]/70'
                : 'text-slate-400 hover:text-slate-200 hover:bg-[#1e3a5f]/10 border border-transparent'
            }`}
          >
